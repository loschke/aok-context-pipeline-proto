...
interface StepExtractProps {
  step: PipelineStep
  /** Files from step 1 (scraping) to use as source */
  scrapingFiles: { name: string; path: string; size: number }[]
  batch: string
  onComplete: () => void
}

export function StepExtract({ step, scrapingFiles, batch, onComplete }: StepExtractProps) {
  ...
  function selectAll() {
    if (selectedFiles.size === scrapingFiles.length) {
      setSelectedFiles(new Set())
    } else {
      setSelectedFiles(new Set(scrapingFiles.map((f) => f.path)))
    }
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold">Quelldateien aus Schritt 1</h3>
            <p className="text-xs text-muted-foreground">
              Waehle die gescrapten Dateien fuer die Baustein-Extraktion.
            </p>
          </div>
          {scrapingFiles.length > 0 && (
            <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={selectAll}>
              {selectedFiles.size === scrapingFiles.length ? "Keine" : "Alle"} auswaehlen
            </Button>
          )}
        </div>
        ...
        {scrapingFiles.length === 0 ? (
          <p className="rounded border border-dashed p-4 text-center text-xs text-muted-foreground">
            Keine Dateien aus Schritt 1 vorhanden. Zuerst Inhalte scrapen.
          </p>
        ) : (
          <div className="flex flex-col gap-1">
            {scrapingFiles.map((file) => (
              <label
                key={file.path}
      ...
      {(isRunning || streamContent) && (
        <div className="rounded border p-4">
          <h4 className="mb-2 flex items-center gap-2 text-xs font-medium text-muted-foreground">
            ...
            AI-Extraktion {isRunning ? "(laeuft...)" : "(abgeschlossen)"}
          </h4>
          <Streamdown
      ...
      {step.files.length > 0 && (
        <div>
          <h4 className="mb-2 text-xs font-medium text-muted-foreground">
            Vorhandene Bausteine ({step.files.length})
          </h4>
          <div className="flex flex-col gap-1">
            {step.files.map((file) => (
              <button
