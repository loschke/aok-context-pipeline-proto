...
export function StepExtractV2({ step, scrapingFiles, cluster, onComplete }: StepExtractV2Props) {
  ...
  const handleExtract = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/pipeline-v2/extract", {
        method: "POST",
  ...
  return (
    <div className="flex flex-col gap-6">
      <SourceFileSelector
        ...
        description="Waehle die gescrapten Dateien fuer die Baustein-Extraktion."
      ...
      >
        {isRunning ? (
          ...
          <><Sparkles className="size-3.5" />Bausteine extrahieren</>
        )}
      </Button>
