...
interface StepTaxonomizeV2Props {
  step: PipelineStepV2
...
export function StepTaxonomizeV2({ step, bausteinFiles, cluster, onComplete }: StepTaxonomizeV2Props) {
  ...
  function selectAll() {
    if (selectedFiles.size === bausteinFiles.length) setSelectedFiles(new Set())
    else setSelectedFiles(new Set(bausteinFiles.map((f) => f.path)))
  }
  ...
  const handleTaxonomize = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/pipeline-v2/taxonomize", {
        method: "POST",
  ...
  return (
    <div className="flex flex-col gap-6">
      <SourceFileSelector
        label="Baustein-Dateien aus Schritt 2"
        description="Waehle die Baustein-Dateien fuer die Taxonomie-Analyse."
        emptyMessage="Keine Baustein-Dateien aus Schritt 2 vorhanden."
        files={bausteinFiles}
      ...
      >
        {isRunning ? (
          ...
          <><Sparkles className="size-3.5" />Taxonomie erstellen</>
        )}
      </Button>
