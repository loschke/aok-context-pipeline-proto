...
import { EXTRACTION_V2_SYSTEM_PROMPT } from "@/lib/pipeline/prompts/extract-v2"
...
export async function POST(req: Request) {
  ...
  const userMessage = [
    `# Baustein-Extraktion fuer Cluster: ${cluster}`,
    "",
    ...
    "---",
    "",
    "Extrahiere alle Wissensbausteine aus den obigen Quelldateien. Verwende die 8 Bausteintypen.",
  ].join("\n")

  const result = streamText({
    model: gateway(aiDefaults.model),
    system: EXTRACTION_V2_SYSTEM_PROMPT,
    messages: [{ role: "user", content: userMessage }],
    maxOutputTokens: 8192,
    temperature: 0.3,
    async onFinish({ text }) {
      if (text) {
        await writePipelineFileV2(`${cluster}/bausteine/${cluster}-bausteine.md`, text)
      }
    },
  })
