...
import { CLUSTER_ANALYSE_SYSTEM_PROMPT } from "@/lib/pipeline/prompts/cluster-analyse"
...
export async function POST(req: Request) {
  ...
  const userMessage = [
    `# Themenanalyse fuer Cluster: ${cluster}`,
    "",
    ...
    "---",
    "",
    "Analysiere alle Quelldateien und erstelle eine thematische Gliederung (Outline) fuer das Cluster-Dokument.",
  ].join("\n")

  const result = streamText({
    model: gateway(aiDefaults.model),
    system: CLUSTER_ANALYSE_SYSTEM_PROMPT,
    messages: [{ role: "user", content: userMessage }],
    maxOutputTokens: 16000,
    temperature: 0.3,
    async onFinish({ text }) {
      if (text) {
        await writeClusterFile(`${cluster}/analyse/${cluster}-outline.md`, text)
      }
    },
  })
