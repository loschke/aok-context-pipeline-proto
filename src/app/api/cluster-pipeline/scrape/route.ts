...
import { writeClusterFile } from "@/lib/pipeline/files-cluster"
...
export async function POST(req: Request) {
  ...
  if (!Array.isArray(urls) || urls.length === 0 || urls.length > 50) {
    return new Response("urls must be an array of 1-50 URLs", { status: 400 })
  }
    ...
    async start(controller) {
      ...
      for (const url of urls) {
        ...
        try {
          ...
          await writeClusterFile(filePath, header + markdown)
